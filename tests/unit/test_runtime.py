...
class RuntimeTest(unittest.TestCase):
    def test_readiness_failure_staleness_recovery_and_shutdown(self) -> None:
        clock = Clock()
        status = RuntimeStatus("1.2.3", "abc", 15, monotonic=lambda: clock.steady, wall_time=lambda: clock.wall)
        self.assertIs(status.readiness(), ReadinessReason.INITIAL_COLLECTION)
        success = successful_result()
        status.record_collection(success, 0.025, True, 1)
    ...
    def test_device_requirement_and_bounded_self_metrics(self) -> None:
        clock = Clock()
        status = RuntimeStatus('1"2', "rev\\1", 15, True, lambda: clock.steady, lambda: clock.wall)
        status.record_collection(successful_result(0), 0.002, True, 1)
        self.assertIs(status.readiness(), ReadinessReason.DEVICE_REQUIRED)
        status.connection_opened()
        status.record_http_request(HttpRoute.METRICS, 200, 0.003)
        status.record_http_request(HttpRoute.OTHER, 404, 0.004)
