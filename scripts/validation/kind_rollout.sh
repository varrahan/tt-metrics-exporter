...
cluster_wait=${KIND_CLUSTER_WAIT:-10m}
rollout_timeout=${KIND_ROLLOUT_TIMEOUT:-10m}
work=$(mktemp -d)
...
kind delete cluster --name "${cluster}" >/dev/null 2>&1 || true
kind create cluster --name "${cluster}" --image "${node_image}" --wait "${cluster_wait}"
...
kubectl rollout status daemonset/tt-metrics-exporter --timeout="${rollout_timeout}"
