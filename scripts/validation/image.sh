...
if test "${SKIP_SUPPLY_CHAIN:-0}" != "1"; then
  docker sbom "${image}" --format spdx-json \
    >"${report_dir}/sbom.spdx.json" 2>"${report_dir}/sbom.log"
  docker scout cves "${image}" --format sarif --output "${report_dir}/vulnerabilities.sarif"
fi
