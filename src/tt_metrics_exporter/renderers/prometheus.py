...
def _device_labels(device: DeviceTelemetry) -> list[tuple[str, object | None]]:
    return [
        ("device", device.id), ("architecture", device.architecture),
        ("board_type", device.board_type), ("pci_bdf", device.pci.bdf),
    ]
...
def _info_labels(device: DeviceTelemetry) -> list[tuple[str, object | None]]:
    ...
    return _device_labels(device) + [
        ("sysfs_path", str(device.sysfs_path)), ("health", device.health),
        ...
        ("pci_device_id", device.pci.device_id), ("pci_driver", device.pci.driver),
        ("pci_class_id", device.pci.class_id), ("pci_revision", device.pci.revision),
        ("pci_subsystem_vendor_id", device.pci.subsystem_vendor_id),
        ("pci_subsystem_device_id", device.pci.subsystem_device_id),
        ("numa_node", device.pci.numa_node), ("iommu_group", device.pci.iommu_group),
...
def _workload_labels(
    device: DeviceTelemetry, workload: MetaliumWorkloadTelemetry
) -> list[tuple[str, object | None]]:
    return _device_labels(device) + [
        ("workload_id", workload.workload_id),
        ("pod_namespace", workload.pod_namespace), ("pod_name", workload.pod_name),
...
class _Renderer:
    ...
    def family(self, name: str, help_text: str, metric_type: str,
               writer: SampleWriter) -> None:
        self.lines += [f"# HELP {name} {help_text}", f"# TYPE {name} {metric_type}"]
        for device in self.devices:
            self.lines.extend(writer(device))
        self.lines.append("")

    def optional(self, name: str, help_text: str, metric_type: str,
                 getter: Callable[[DeviceTelemetry], object | None]) -> None:
    ...
    def workload(self, name: str, help_text: str,
                 writer: Callable[[DeviceTelemetry, MetaliumWorkloadTelemetry], str | None]
                 ) -> None:
        def samples(device: DeviceTelemetry) -> Iterable[str]:
...
def render_prometheus(devices: list[DeviceTelemetry]) -> str:
    ...
    for name, help_text in (
        ...
        out.family(name, help_text, "gauge", lambda d, n=name: (
            f"{n}{{{_labels(_info_labels(d))}}} 1",
        ))

    def firmware_labels(d: DeviceTelemetry) -> list[tuple[str, object]]:
        return _device_labels(d) + [
            ("serial", d.firmware.serial), ("card_type", d.firmware.card_type),
    ...
    out.family("tt_firmware_info",
               "Firmware-reported Tenstorrent board and firmware identity.", "gauge",
               lambda d: (f"tt_firmware_info{{{_labels(firmware_labels(d))}}} 1",))

    def clocks(device: DeviceTelemetry) -> Iterable[str]:
        for clock, value in (("ai", device.firmware.ai_clock_mhz),
                             ("axi", device.firmware.axi_clock_mhz),
                             ("arc", device.firmware.arc_clock_mhz)):
            if value is not None:
                yield ("tt_firmware_clock_frequency_mhz{"
                       f"{_labels(_device_labels(device) + [('clock', clock)])}}} {value}")
    out.family("tt_firmware_clock_frequency_mhz",
               "Firmware-reported Tenstorrent clock frequencies in MHz.", "gauge", clocks)
    ...
    def resources(device: DeviceTelemetry) -> Iterable[str]:
        for item in device.pci_resources:
            labels = _device_labels(device) + [("resource", item.index),
                ("start", hex(item.start)), ("end", hex(item.end)), ("flags", hex(item.flags))]
    ...
    out.family("tt_power_state_info", "Runtime power-management state labels for the device.", "gauge",
               lambda d: (f"tt_power_state_info{{{_labels(_device_labels(d) + [('runtime_status', d.power.runtime_status), ('control', d.power.control), ('runtime_enabled', d.power.runtime_enabled)])}}} 1",))
    ...
    def workload_sample(name: str, device: DeviceTelemetry,
                        workload: MetaliumWorkloadTelemetry, value: object) -> str:
        return f"{name}{{{_labels(_workload_labels(device, workload))}}} {value}"

    out.workload("tt_metalium_workload_active", "Whether a fresh TT-Metalium workload profiler sample reported activity.",
                 lambda d, w: workload_sample("tt_metalium_workload_active", d, w, int(not w.stale and (w.active or 0) != 0)))
    out.workload("tt_metalium_workload_profile_stale", "Whether the TT-Metalium workload profiler sample exceeded its freshness window.",
                 lambda d, w: workload_sample("tt_metalium_workload_profile_stale", d, w, int(w.stale)))

    def optional_workload(name: str, help_text: str, attribute: str,
                          fresh_only: bool = True) -> None:
    ...
    def occupancy(d: DeviceTelemetry, w: MetaliumWorkloadTelemetry) -> str | None:
        if w.stale or w.tensix_cores_used is None or not w.tensix_cores_total or w.tensix_cores_total < 0:
            return None
        return workload_sample("tt_metalium_workload_core_occupancy_ratio", d, w,
                               format(w.tensix_cores_used / w.tensix_cores_total, ".6g"))
    ...
    def janitor(device: DeviceTelemetry) -> Iterable[str]:
        item = device.janitor
        presence = [item.state, item.quarantine_reason, item.last_scrub_status, item.last_reset_status,
                    item.scrub_count, item.reset_count, item.last_scrub_timestamp_seconds, item.last_reset_timestamp_seconds]
        if any(value is not None for value in presence):
            labels = _device_labels(device) + [("state", item.state), ("quarantine_reason", item.quarantine_reason),
                ("last_scrub_status", item.last_scrub_status), ("last_reset_status", item.last_reset_status)]
