...
def _response(status: int, content_type: str, body: str, headers: dict[str, str] | None = None) -> Response:
    response_headers = {
...
class _ContractMiddleware:
    def __init__(self, app: object, status: RuntimeStatus | None, maximum_header_bytes: int) -> None:
    ...
    async def __call__(self, scope: dict, receive: Callable, send: Callable) -> None:
        ...
        try:
            ...
            if self.status:
                self.status.record_http_request(_route(scope["path"]), status_code, time.monotonic() - started)


def _protocol(status: RuntimeStatus | None, logger: Logger | None, config: HttpServerConfig) -> type[H11Protocol]:
    ...
    class BoundedH11Protocol(H11Protocol):
        def connection_made(self, transport: asyncio.Transport) -> None:
            ...
            self._initial_request_timeout = self.loop.call_later(config.request_deadline, self._request_timed_out)
...
class HttpServer:
    def __init__(self, listen_address: str, port: int, metrics_provider: Callable[[], str], devices_provider: Callable[[], str], runtime_status: RuntimeStatus | None = None, logger: Logger | None = None, config: HttpServerConfig | None = None) -> None:
    ...
    def _application(self) -> _ContractMiddleware:
        async def metrics(_: Request) -> Response:
            return _response(200, "text/plain; version=0.0.4; charset=utf-8", self.metrics_provider())
        ...
        async def ready(_: Request) -> Response:
            readiness = self.runtime_status.readiness() if self.runtime_status else ReadinessReason.INITIAL_COLLECTION
        ...
        app = Starlette(
            routes=[
                ...
                Route("/v1/devices", devices),
                Route("/healthz", health),
                Route("/readyz", ready),
                Route("/{path:path}", missing),
            ]
        )
        return _ContractMiddleware(app, self.runtime_status, self.config.maximum_header_bytes)
    ...
    def serve(self, running: Event) -> int:
        ...
        configuration = uvicorn.Config(
            ...
            log_config=None,
            log_level="critical",
            backlog=self.config.queue_depth,
        ...
        if self.logger:
            self.logger.log(LogLevel.INFO, "http.listen_started", "HTTP listener started", {"address": self.address, "port": self.port})
        try:
            self._server.run()
        except (OSError, SystemExit):
            if self.logger:
                self.logger.log(LogLevel.ERROR, "http.request_rejected", "listener activation failed")
            return 1
