...
class SysfsCollector(DeviceResourceReader, SysfsReader):
    ...
    def collect(self) -> CollectionResult:
        ...
        state = StateIngestor.open(self.config, result)
        try:
            try:
            ...
            for entry in entries:
                ...
                device = DeviceTelemetry(
                    id=entry.name, sysfs_path=Path(entry.path)
                )
    ...
    def _populate_device(
        self, device: DeviceTelemetry, diagnostics: SourceDiagnostics
    ) -> None:
        ...
        device.health = self._read_first_text(
            device.sysfs_path, ("health", "status", "device_status"), diagnostics
        )
        device.character_device = self._read_character_device(
            device.sysfs_path, diagnostics
        )
        ...
        if device.firmware.card_type is not None:
            device.board_type = self._normalize_card_type(
                device.firmware.card_type
            )
        ...
        device.hwmon_sensors = self._read_hwmon_sensors(
            device.sysfs_path, diagnostics
        )
        device.power = self._read_power_info(device.sysfs_path, diagnostics)
        resource = self._read_text(
            device.sysfs_path / "device/resource", diagnostics
        )
        if resource is not None:
            device.pci_resources = parse_pci_resources(resource)
        if self.config.collect_pcie_counters:
            device.pcie_counters = self._read_pcie_counters(
                device.sysfs_path, diagnostics
            )
        self._populate_memory(device, diagnostics)
        self._populate_tensix(device, diagnostics)
        self._populate_health(device, diagnostics)
        device.interconnect_links = self._read_interconnect_links(
            device.sysfs_path, diagnostics
        )

    def _read_uevent_field(
        self, root: Path, field: str, diagnostics: SourceDiagnostics
    ) -> str | None:
    ...
    def _read_character_device(
        self, root: Path, diagnostics: SourceDiagnostics
    ...
    def _read_pci_info(
        ...
        if parsed_iommu is None:
            parsed_iommu = parse_int64(
                self._read_text(pci_root / "iommu_group", diagnostics) or ""
            )
        return PciInfo(
            bdf=bdf,
            driver=driver,
            ...
            subsystem_vendor_id=self._read_text(
                pci_root / "subsystem_vendor", diagnostics
            ),
            subsystem_device_id=self._read_text(
                pci_root / "subsystem_device", diagnostics
            ),
            numa_node=parse_int64(
                self._read_text(pci_root / "numa_node", diagnostics) or ""
            ),
            iommu_group=parsed_iommu,
            current_link_speed=self._read_text(
                pci_root / "current_link_speed", diagnostics
            ),
            current_link_width=self._read_int(
                pci_root / "current_link_width", diagnostics
            ),
            max_link_speed=self._read_text(
                pci_root / "max_link_speed", diagnostics
            ),
            max_link_width=self._read_int(
                pci_root / "max_link_width", diagnostics
            ),
    ...
    def _populate_firmware(
        ...
        firmware.thermal_trip_count = self._read_int(
            root / "tt_therm_trip_count", diagnostics
        )
                    ...
                    sensors.append(
                        HwmonSensor(
                            ...
                            label=self._read_text(
                                directory / f"{prefix}_label", diagnostics
                            ),
    ...
    def _read_power_info(
        self, root: Path, diagnostics: SourceDiagnostics
    ) -> PowerInfo:
        power_root = root / "power"
        return PowerInfo(
            runtime_status=self._read_text(
                power_root / "runtime_status", diagnostics
            ),
            control=self._read_text(power_root / "control", diagnostics),
            runtime_enabled=self._read_text(
                power_root / "runtime_enabled", diagnostics
            ),
            runtime_active_time_ms=parse_int64(
                self._read_text(power_root / "runtime_active_time", diagnostics)
                or ""
            ),
            runtime_suspended_time_ms=parse_int64(
                self._read_text(power_root / "runtime_suspended_time", diagnostics)
                or ""
            ),
            runtime_usage=parse_int64(
                self._read_text(power_root / "runtime_usage", diagnostics) or ""
            ),
            runtime_active_kids=parse_int64(
                self._read_text(power_root / "runtime_active_kids", diagnostics)
                or ""
            ),
            autosuspend_delay_ms=parse_int64(
                self._read_text(power_root / "autosuspend_delay_ms", diagnostics)
                or ""
            ),
        )

    def _read_pcie_counters(
        self, root: Path, diagnostics: SourceDiagnostics
    ) -> list[NamedCounter]:
    ...
    def _populate_health(
        ...
        health.fault_code = self._read_first_text(
            root, ("fault_code", "device_fault_code", "last_fault_code"), diagnostics
        )
        ...
        health.reset_required = self._read_first_boolish(
            root, ("reset_required", "needs_reset", "requires_reset"), diagnostics
        )
        health.oom_fault_count = self._read_first_int(
            root, ("oom_fault_count", "oom_count", "out_of_memory_count"), diagnostics
        )
        health.hang_fault_count = self._read_first_int(
            root, ("hang_fault_count", "hang_count", "device_hang_count"), diagnostics
        )

    def _read_interconnect_links(
        self, root: Path, diagnostics: SourceDiagnostics
        ...
        for base, default_type in (
            ...
            for entry in entries:
                links.append(
                    InterconnectLink(
                        name=entry.name,
                        type=self._read_first_text(
                            entry, ("type", "link_type"), diagnostics
                        )
                        or default_type,
                        state=self._read_first_text(
                            entry, ("state", "status"), diagnostics
                        ),
                        ...
                        ring_id=self._read_first_text(
                            entry, ("ring_id", "ring"), diagnostics
                        ),
                    )
                )
