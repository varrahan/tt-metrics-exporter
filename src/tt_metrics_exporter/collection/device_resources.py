...
class DeviceResourceReader:
    def _populate_memory(self, device: DeviceTelemetry, diagnostics: SourceDiagnostics) -> None:
        root, memory = device.sysfs_path, device.memory
        usage = self._read_first_text(
            root,
            ("memory_usage", "dram_usage", "device_memory_usage", "tt_memory_usage"),
            diagnostics,
        )
        ...
        if memory.used_bytes is None:
            memory.used_bytes = self._read_first_byte(
                root,
                (
                    "memory_used_bytes",
                    "dram_used_bytes",
        ...
        if memory.total_bytes is None:
            memory.total_bytes = self._read_first_byte(
                root,
                (
                    "memory_total_bytes",
        ...
        memory.available_bytes = self._read_first_byte(
            root,
            ("memory_available_bytes", "dram_available_bytes", "device_memory_available_bytes"),
            diagnostics,
        )
        memory.bandwidth_bytes_per_second = self._read_first_byte(
            root,
            (
        ...
        memory.type = self._read_first_text(root, ("memory_type", "dram_type", "gddr_type"), diagnostics)
        memory.controller_layout = self._read_first_text(
            root,
            (
                "gddr_controller_layout",
                "dram_controller_layout",
                "memory_controller_layout",
            ),
            diagnostics,
        )
        memory.controller_count = self._read_first_int(
            root,
            (
                "gddr_controller_count",
                "dram_controller_count",
                "memory_controller_count",
            ),
            diagnostics,
        )
        memory.controllers_per_asic = self._read_first_int(
            root,
            (
                "gddr_controllers_per_asic",
                "dram_controllers_per_asic",
                "memory_controllers_per_asic",
            ),
            diagnostics,
        )
        memory.channel_count = self._read_first_int(
            root,
            (
                "dram_channel_count",
                "gddr_channel_count",
    ...
    def _populate_tensix(self, device: DeviceTelemetry, diagnostics: SourceDiagnostics) -> None:
        root, tensix = device.sysfs_path, device.tensix
        tensix.used = self._read_first_int(
            root,
            (
                "tensix_cores_used",
                "tensix_used",
        ...
        tensix.available = self._read_first_int(
            root,
            (
                "tensix_cores_available",
                "tensix_available",
                "available_tensix_cores",
            ),
            diagnostics,
        )
        tensix.total = self._read_first_int(
            root,
            (
                "tensix_cores_total",
                "total_tensix_cores",
        ...
        tensix.mesh_rows = self._read_first_int(
            root,
            (
                "tensix_mesh_rows",
                "tensix_grid_rows",
        ...
        tensix.mesh_cols = self._read_first_int(
            root,
            (
                "tensix_mesh_cols",
                "tensix_grid_cols",
        ...
        tensix.topology = self._read_first_text(root, ("tensix_topology", "tensix_layout"), diagnostics)
        tensix.active_regions = self._read_first_text(
            root,
            ("tensix_active_regions", "active_core_ranges", "active_core_grids"),
            diagnostics,
        )
        if any(
            value is not None
